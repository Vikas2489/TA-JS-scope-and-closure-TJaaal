...
function objOfMatches(array1, array2, callback) {
 return  array1.reduce(function (acc,elm,i) {
...
return arrVals.reduce(function(acc,cv,i){
  acc[cv] = arrCallbacks.map(function(fn){return fn(cv);})
return acc;
},{})
...
function objOfMatchesWithArray(array1, array2, callback) {
 return array1.reduce(function (acc,cv, i){
  let value = callback.reduce(function (acc,fn){
acc =  fn(acc);
return acc;
    },cv);
...
return array1.reduce(function(acc,cv){
let value = callback.reduce(function(acc,fn){
 acc.push(fn(cv));
 return acc;
},[]);
...
```js
function schedule(arrayOfFn, secondsArray){
  if(arrayOfFn.length !== secondsArray.length){
 alert('Invalid Input');
  } 
   return arrayOfFn.forEach((fn, i)=>{
     setTimeout(fn,secondsArray[i] * 1000);
    })
